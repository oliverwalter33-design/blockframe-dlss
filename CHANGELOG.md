...
- Removed forced 16x anisotropic filtering and preserved the original or
  user-selected sampler anisotropy.
- Removed the Cutout-terrain exclusion from the DLSS mip correction.
- Removed global per-frame Streamline tagging and the volatile
  `eOnlyValidNow` clone path.
- Replaced the R8_UNORM history mask with an RGBA8_UNORM mask supported by
  Streamline 2.12.
- Removed duplicate native-terrain pipeline registration when Sodium is active.
- Removed diagnostic image bindings and branches from the release motion shader.
- Replaced the raw full-detail Bricks distance extension with an efficient
  cached far-mesh path.

### Fixed

- Terrain mip bias now uses the real render and output dimensions:
  original sampler bias plus `log2(renderWidth / outputWidth) - 1`.
- Fixed arbitrary window sizes, live resizing, minimize/restore and DLSS/DLAA
  mode changes.
- Fixed complete Vulkan sampler cloning while adjusting only `mipLodBias`.
- Fixed distant Solid and alpha-tested Cutout terrain detail for the supported
  Minecraft, Sodium, Milkshade and BlockFrame pipelines.
- Fixed local-player disocclusion history handling with a tightly scoped
  `BiasCurrentColorHint` mask.
- Fixed Streamline resource lifetime by using local `eValidUntilEvaluate` tags.
- Fixed Vulkan buffer-device-address promotion and
  `shaderStorageImageWriteWithoutFormat` negotiation.
- Fixed motion descriptor accounting and GPU-safe sampler/cache cleanup.
- Fixed distant Bricks `composite_block` geometry disappearing at the normal
  64-block block-entity-renderer limit. Roofs, facades, bars and other
  microblock structures now remain visible through Minecraft's effective
  render distance.

### Added

- Added an exact block-atlas and terrain-pipeline allowlist for five Solid and
  four Cutout pipeline identifiers.
- Added a device- and generation-scoped sampler cache.
- Added an RGBA8_UNORM local-player `BiasCurrentColorHint` texture with complete
  Vulkan/Streamline lifecycle and resize support.
- Added previous-frame projected articulated-player rectangles and a
  world-pixel-only history-rejection gate.
- Added an exactly pinned Bricks 1.0.1 compatibility module.
- Added cached, component-safe greedy Bricks far meshes with preserved material,
  silhouette, texture-atlas region, UV rotation and translucency.
- Added frame-wide texture/translucency batching, immutable NeoForge geometry
  submissions and far-to-near translucent-quad ordering.
- Added automatic far-mesh cache invalidation and expanded artifact, UV,
  lifecycle, shader, Vulkan and regression coverage.
